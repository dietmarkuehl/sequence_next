...
struct state_helper {
    ...
    using state_t = decltype(::beman::execution::connect(
        ::std::declval<sender_t>(), ::std::declval<R>()
    ));
    state_t state;
    state_helper(S&& s, R&& r)
        : state(::beman::execution::connect(::std::forward<S>(s), ::std::forward<R>(r)))
    {
    }
};
...
struct conditional_element_t {
    ...
    struct sender {
        ...
        auto get_completion_signatures(Env const& e) const noexcept {
        ...
        struct state {
            ...
            using state_variant = ::beman::execution::value_types_of_t<
                Sender_up,
                env_t,
                just_state_helper_t,
                variant_with_monostate
            >;
            ...
            struct receiver_up {
                ...
                auto set_value(A&&... a) && noexcept -> void {
                    if (this->st->predicate(a...)) {
                        ...
                        using down_state_t = decltype(state_helper(
                            std::move(sndr),
                            std::move(this->st->receiver)
                        ));
                        auto& s{this->st->state_down.template emplace<down_state_t>(
                            std::move(sndr),
                            std::move(this->st->receiver)
                        )};
            ...
            using inner_state_t = decltype(::beman::execution::connect(
                std::declval<Sender_up>(), std::declval<receiver_up>()
                ));
            ...
            state(R&& rcvr, U&& u, F&& f, P&&p)
                ...
                , inner_state(::beman::execution::connect(
                    ::std::forward<U>(u),
                    receiver_up{this}
                ))
