...
struct then_each_t : ::beman::execution::sender_adaptor_closure<then_each_t> {
    ...
    struct sender {
        ...
        struct state_base {
            ...
            state_base(R&& r, F&& f) : rcvr(std::forward<R>(r)), fun(std::forward<F>(f)) {}
        };
        ...
        auto connect(Receiver&& rcvr) {
            return state<std::remove_cvref_t<Receiver>>(
                std::move(this->fun), std::move(sender), std::forward<Receiver>(rcvr));
        }
    };

    template <::beman::execution::sender Sender, typename Fun>
    sender<::std::remove_cvref_t<Sender>, ::std::remove_cvref_t<Fun>> operator()(Sender&& sndr, Fun&& fun) const {
