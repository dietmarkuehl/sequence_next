...
struct iota {
    ...
    struct state {
        ...
        using set_next_sender = decltype(
            set_next(std::declval<Receiver&>(), ::beman::execution::just(0))
        );
        using set_next_state = decltype(
            ::beman::execution::connect(std::declval<set_next_sender>(), std::declval<set_next_receiver>())
        );
        struct state_helper {
        ...
        void produce_next() {
            if (this->next != this->end) {
                this->helper.emplace(
                  set_next(this->rcvr, ::beman::execution::just(this->next++)),
