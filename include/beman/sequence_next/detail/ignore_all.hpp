...
struct ignore_all_t : ::beman::execution::sender_adaptor_closure<ignore_all_t> {
    struct state_base {
    ...
    struct receiver {
        ...
        auto set_value() && noexcept { this->state->complete(); }
    };
    ...
    struct state : state_base {
        ...
        state(S&& s, R&& r)
            : rcvr(std::forward<R>(r)), inner(::beman::execution::connect(std::forward<S>(s), receiver{this})) {}
        void start() & noexcept { ::beman::execution::start(this->inner); }
        void complete() override { ::beman::execution::set_value(std::move(this->rcvr)); }
    };
    ...
    struct sender {
        ...
        auto connect(Receiver&& rcvr) {
            return state<Sender, std::remove_cvref_t<Receiver>>(std::move(this->sender), std::forward<Receiver>(rcvr));
        }
    };
